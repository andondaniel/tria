...
function tria_widgets_init() {
	register_sidebar( array(
		...
		register_sidebar( array(
		'name'          => __( 'Sidebar Tuesday Conference', 'tria' ),
		'id'            => 'sidebar-tuesday-conference',
		'description'   => '',
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
...
function tria_scripts() {
	...
	wp_enqueue_script( 'tria-lazy-script', get_template_directory_uri() . '/js/jquery.lazyload.min.js', array(), '20120206', true );
	if (is_home()) {
		wp_enqueue_script( 'tria-blog-script', get_template_directory_uri() . '/js/blog.js', array('jquery'), '20120206', true );
	}
	wp_enqueue_script( 'tria-custom-script', get_template_directory_uri() . '/js/custom.js', array(), '20120206', true );
	wp_localize_script( 'tria-custom-script', 'TRIA_OBJ', array(
		'site_url' => site_url() . '/',
...
function wen_widgets_init() {
    ...
    register_widget('WEN_Block_Blue_Widget');
}
